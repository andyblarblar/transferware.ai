...
class APICache:
    def __init__(self, directory: str | None = ""):
        ...
        self.db_file = os.path.join(directory, 'db.csv') # csv cache file
        self.assets_dir = os.path.join(directory, 'assets') # directory for images

    def ensure_cache(self):
        patterns = get_transferware_patterns() # get patterns from API
        df = pl.DataFrame(patterns) # convert to polars dataframe
        df.write_csv(self.db_file) # write dataframe to csv
        if not os.path.exists(self.assets_dir):
            os.makedirs(self.assets_dir) # create assets directory
